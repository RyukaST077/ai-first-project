# Excel(.xlsx/xls) / Word(.docx) -> Markdown 変換（Windows向け）
...
```mermaid
flowchart LR
    A[既存設計書<br>（.xlsx/.xls or .docx）] -->|Pythonコマンド<br>convert_to_md.cmd| B[Cline用一時的な情報<br>`temp_design/`]
...
## 使い方（エクスプローラー上で完結します）
1. このリポジトリをクローン
2. `設計書/` フォルダに `.xlsx`、`.xls` や `.docx` ファイルを入れる（サブフォルダOK）
3. `run.cmd` を実行（ダブルクリックで実行可）

出力は `converted_md_design/` に、`設計書/` と同じサブディレクトリ構造で保存されます。
- 各xlsx/xlsファイル：「シートごとにmdファイル」を生成
