...
- **再開可能性(Resumability)**：出力対象のファイルが既に存在し、かつ中身が空でなければ「作成済み」とみなし、Stepをスキップするかユーザーに確認する（上書き指示がある場合を除く）。

# 手順（ステップバイステップ）
## Step 1: テンプレ把握と入力確認
1) `/templates/design` の各テンプレファイルを確認。
2) ユーザーに対象の情報元ファイル、ドメイン、画面一覧、バッチ一覧を確認。
3) **再開確認**: 既に一部の出力ファイルが存在するか確認し、途中から再開するか最初に確認する。
...
* **再開:** 既存の画面ファイルがある場合、その画面IDの作成はスキップする。
...
* **再開:** 既存のテーブル定義ファイルがある場合、そのテーブルの作成はスキップする。
...
* **再開:** 既存のバッチ処理定義ファイルがある場合、そのバッチの作成はスキップする。
