...
3. 各コミットタスクは必ず以下のセクションで出力する：

   * **A. 事前確認**（既存コード/設定/規約、影響範囲、進捗確認）
...
9. **ターミナルでコマンドを実行する際は下記のテンプレに従って、ファイルにログを保存し、末尾200行のみを出力すること**
```powershell
...
```
12. 設計書で指定された ID および定数は、大文字・小文字・記号を含め、設計書の記載と完全に一致するように実装すること。
...
3. **開始コミットタスク ( C-000 )の決定ルール**
   ...
   * ただし、以下の場合は「適切な開始地点」を調整してよい：
     ...
     * 「不明」判定のコミットタスク ( C-000 )がある：→ そのコミットタスク ( C-000 )から着手し、A.事前確認で状況を確定させる。
...
* コミットタスク ( C-000 )の目的と成功条件を1〜3行で要約。
...
2. 次に、そのコミットタスク ( C-000 )について **A.事前確認 → B → C → D → E** の順で完了させてください。
3. コミットタスク ( C-000 )完了後はタスクリストにチェックを付け、次のコミットタスク ( C-000 )へ進んでください。
