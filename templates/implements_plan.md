...
## Phase 0: キックオフ & 設計固定（最短で）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-001 | 全体 | - | 要件/スコープ確定（In/Out） | なし | | |
| P0 | T-002 | 全体 | T-001 | 画面・機能カタログ確定（3章） | なし | | |
| P0 | T-003 | 全体 | T-002 | 依存関係マトリクス確定（3.2） | なし | | |
| P0 | T-004 | API | T-002 | API I/F（ざっくり）固定（エンドポイント一覧、入出力例） | なし | | |
| P0 | T-005 | 運用 | - | ブランチ運用/レビュー運用の合意（2章） | なし | | |

## Phase 1: プロジェクトセットアップ（開発土台）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-006 | Repo | - | リポジトリ作成/権限設定 | なし | | |
| P0 | T-007 | Env | T-006 | 環境構築（local/dev/stg/prodの雛形） | なし | | |
| P1 | T-008 | CI | T-006 | CI（lint/test/build）導入 | なし | | |
| P1 | T-009 | Base | T-006 | コーディング規約/フォーマッタ導入 | なし | | |
| P2 | T-010 | Obs | - | 監視/ログ/エラー通知の最小導入（可能なら） | なし | | |
| P1 | T-011 | Data | - | シード/ダミーデータ方針決定 & 最小整備 | なし | | |

## Phase 2: 画面遷移の骨格（並行開発を可能にする）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-012 | Router | T-007 | ルーティング/ナビゲーション雛形（共通レイアウト） | なし | | |
| P0 | T-013 | Screen | T-012 | 画面コンテナ（空画面）を全カタログ分だけ先に作成（S-xx） | なし | | |
| P1 | T-014 | UI | T-012 | モック画面枠（S-MOCK）と共通コンポーネント（ボタン/フォーム等） | なし | | |
| P0 | T-015 | Auth | T-012 | 認可が絡む場合のガード（ログイン前後の遷移） | なし | | |

## Phase 3: ドメイン基盤（API/DB/共通モジュール）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-016 | DB | T-007 | DBスキーマ最小セット（主要テーブル） | なし | | |
| P0 | T-017 | API | T-004 | APIの型/DTO/バリデーション方針 | なし | | |
| P0 | T-018 | Auth | Phase 2 | 認証（F-01）※依存が多いなら最優先 | なし | | |
| P1 | T-019 | Base | Phase 2 | 共通エラーハンドリング/ローディング/リトライ | なし | | |
...
| 優先 | タスクID | 対象   | 依存   | 内容（ざっくり）             | モック要否    | 担当 | PR/Issue |
| -- | ----- | ---- | ---- | -------------------- | -------- | -- | -------- |
| P0 | T-020  | F-01 | -    | 認証（ログイン/ログアウト/ガード）   | なし       |    |          |
| P0 | T-021  | S-03 | T-?? | 商品一覧画面（API接続 or モック） | API未完なら要 |    |          |
| P1 | T-022  | S-04 | T-021 | 商品詳細画面（遷移/表示）        | 遷移先未完なら要 |    |          |
...
| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-023 | 全体 | Phase 4 | 主要導線の結合（ログイン→主要機能→完了） | なし | | |
| P0 | T-024 | 全体 | T-023 | モック除去（S-MOCKの置換） | なし | | |
| P1 | T-025 | 全体 | T-023 | 権限/境界値/エラー系の統合 | なし | | |
| P2 | T-026 | Perf | - | パフォーマンス/ページング/キャッシュ（必要なら） | なし | | |
| P1 | T-027 | Spec | - | 仕様の微修正（変更履歴を残す） | なし | | |

## Phase 6: 品質担保（テスト/セキュリティ/運用）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-028 | QA | Phase 5 | テスト観点作成（最低限のE2E/シナリオ） | なし | | |
| P1 | T-029 | QA | T-028 | 重要機能の回帰テスト手順 | なし | | |
| P1 | T-030 | Sec | - | セキュリティ（脆弱性スキャン/権限確認/ログの秘匿） | なし | | |
| P2 | T-031 | Ops | - | 運用手順（障害時、問い合わせ対応、ログの見方） | なし | | |
| P1 | T-032 | Docs | - | リリースノート草案 | なし | | |

## Phase 7: リリース準備 & リリース

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-033 | Prod | Phase 6 | リリース判定（Go/No-Go条件） | なし | | |
| P0 | T-034 | DB | - | マイグレーション手順/リハーサル（stgで） | なし | | |
| P0 | T-035 | Prod | - | 本番リリース手順（担当/時間/ロールバック） | なし | | |
| P1 | T-036 | Obs | - | 監視強化（リリース直後のアラート） | なし | | |
| P0 | T-037 | Prod | T-033 | リリース実施 | なし | | |
| P0 | T-038 | Prod | T-037 | 初動確認（主要導線/メトリクス） | なし | | |

## Phase 8: リリース後

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P1 | T-039 | Team | - | 振り返り（KPT）& 次スプリント改善 | なし | | |
| P2 | T-040 | Code | - | 技術的負債の棚卸し | なし | | |
| P1 | T-041 | Ops | - | 運用課題の整理（アラート過多、ログ不足等） | なし | | |
