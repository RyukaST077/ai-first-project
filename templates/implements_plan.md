...
- [T-0-01] 要件/スコープ確定（In/Out）
- [T-0-02] 画面・機能カタログ確定（3章）
- [T-0-03] 依存関係マトリクス確定（3.2）
- [T-0-04] API I/F（ざっくり）固定（エンドポイント一覧、入出力例）
- [T-0-05] ブランチ運用/レビュー運用の合意（2章）

## Phase 1: プロジェクトセットアップ（開発の土台）
- [T-1-01] リポジトリ作成/権限設定
- [T-1-02] 環境構築（local/dev/stg/prodの雛形）
- [T-1-03] CI（lint/test/build）導入
- [T-1-04] コーディング規約/フォーマッタ導入
- [T-1-05] 監視/ログ/エラー通知の最小導入（可能なら）
- [T-1-06] シード/ダミーデータ方針決定 & 最小整備

## Phase 2: 画面遷移の骨格（並行開発を可能にする）
- [T-2-01] ルーティング/ナビゲーション雛形（共通レイアウト）
- [T-2-02] 画面コンテナ（空画面）を全カタログ分だけ先に作成（S-xx）
- [T-2-03] モック画面枠（S-MOCK）と共通コンポーネント（ボタン/フォーム等）
- [T-2-04] 認可が絡む場合のガード（ログイン前後の遷移）

## Phase 3: ドメイン基盤（API/DB/共通モジュール）
- [T-3-01] DBスキーマ最小セット（主要テーブル）
- [T-3-02] APIの型/DTO/バリデーション方針
- [T-3-03] 認証（F-01）※依存が多いなら最優先
- [T-3-04] 共通エラーハンドリング/ローディング/リトライ
...
| -- | ----- | ---- | ---- | -------------------- | -------- | -- | -------- |
| P0 | T-4-01 | F-01 | -    | 認証（ログイン/ログアウト/ガード）   | なし       |    |          |
| P0 | T-4-02 | S-03 | T-?? | 商品一覧画面（API接続 or モック） | API未完なら要 |    |          |
| P1 | T-4-03 | S-04 | T-4-02 | 商品詳細画面（遷移/表示）        | 遷移先未完なら要 |    |          |

※「モック要否」には「未実装の遷移先」「未完成API」どちらが理由かも書く

## Phase 5: 結合 & 仕様調整（バグ潰しが主）
- [T-5-01] 主要導線の結合（ログイン→主要機能→完了）
- [T-5-02] モック除去（S-MOCKの置換）
- [T-5-03] 権限/境界値/エラー系の統合
- [T-5-04] パフォーマンス/ページング/キャッシュ（必要なら）
- [T-5-05] 仕様の微修正（変更履歴を残す）

## Phase 6: 品質担保（テスト/セキュリティ/運用）
- [T-6-01] テスト観点作成（最低限のE2E/シナリオ）
- [T-6-02] 重要機能の回帰テスト手順
- [T-6-03] セキュリティ（脆弱性スキャン/権限確認/ログの秘匿）
- [T-6-04] 運用手順（障害時、問い合わせ対応、ログの見方）
- [T-6-05] リリースノート草案

## Phase 7: リリース準備 & リリース
- [T-7-01] リリース判定（Go/No-Go条件）
- [T-7-02] マイグレーション手順/リハーサル（stgで）
- [T-7-03] 本番リリース手順（担当/時間/ロールバック）
- [T-7-04] 監視強化（リリース直後のアラート）
- [T-7-05] リリース実施
- [T-7-06] 初動確認（主要導線/メトリクス）

## Phase 8: リリース後
- [T-8-01] 振り返り（KPT）& 次スプリント改善
- [T-8-02] 技術的負債の棚卸し
- [T-8-03] 運用課題の整理（アラート過多、ログ不足等）
