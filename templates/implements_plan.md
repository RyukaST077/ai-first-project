...
## Phase 3: 画面遷移の骨格（並行開発を可能にする）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-011 | Router | T-002 | ルーティング/ナビゲーション雛形（共通レイアウト） | なし | | |
| P0 | T-012 | Screen | T-011 | 画面コンテナ（空画面）を全カタログ分だけ先に作成（S-xx） | なし | | |
| P1 | T-013 | UI | T-011 | モック画面枠（S-MOCK）と共通コンポーネント（ボタン/フォーム等） | なし | | |
| P0 | T-014 | Auth | T-011 | 認可が絡む場合のガード（ログイン前後の遷移） | なし | | |

## Phase 4: ドメイン基盤（API/共通モジュール）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-015 | API | T-004, Phase 2 | APIの型/DTO/バリデーション方針 | なし | | |
| P0 | T-016 | Auth | Phase 2, Phase 3 | 認証（F-01）※依存が多いなら最優先 | なし | | |
| P1 | T-017 | Base | Phase 3 | 共通エラーハンドリング/ローディング/リトライ | なし | | |

## Phase 5: 機能実装（依存順で進める）
...
## Phase 7: 品質担保（テスト/セキュリティ/運用）

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-026 | QA | Phase 6 | テスト観点作成（最低限のE2E/シナリオ） | なし | | |
| P1 | T-027 | QA | T-026 | 重要機能の回帰テスト手順 | なし | | |
| P1 | T-028 | Sec | - | セキュリティ（脆弱性スキャン/権限確認/ログの秘匿） | なし | | |
| P2 | T-029 | Ops | - | 運用手順（障害時、問い合わせ対応、ログの見方） | なし | | |
| P1 | T-030 | Docs | - | リリースノート草案 | なし | | |

## Phase 8: リリース準備 & リリース

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P0 | T-031 | Prod | Phase 7 | リリース判定（Go/No-Go条件） | なし | | |
| P0 | T-032 | DB | - | マイグレーション手順/リハーサル（stgで） | なし | | |
| P0 | T-033 | Prod | - | 本番リリース手順（担当/時間/ロールバック） | なし | | |
| P1 | T-034 | Obs | - | 監視強化（リリース直後のアラート） | なし | | |
| P0 | T-035 | Prod | T-031 | リリース実施 | なし | | |
| P0 | T-036 | Prod | T-035 | 初動確認（主要導線/メトリクス） | なし | | |

## Phase 9: リリース後

| 優先 | タスクID | 対象 | 依存 | 内容（ざっくり） | モック要否 | 担当 | PR/Issue |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| P1 | T-037 | Team | - | 振り返り（KPT）& 次スプリント改善 | なし | | |
