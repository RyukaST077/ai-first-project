...
- 画面・機能カタログ確定（3章）
- 依存関係マトリクス確定（3.2）
- API I/F（ざっくり）固定（エンドポイント一覧、入出力例）
- ブランチ運用/レビュー運用の合意（2章）
...
- コーディング規約/フォーマッタ導入
- 監視/ログ/エラー通知の最小導入（可能なら）
- シード/ダミーデータ方針決定 & 最小整備

## Phase 2: 画面遷移の骨格（並行開発を可能にする）
- ルーティング/ナビゲーション雛形（共通レイアウト）
- 画面コンテナ（空画面）を全カタログ分だけ先に作成（S-xx）
- モック画面枠（S-MOCK）と共通コンポーネント（ボタン/フォーム等）
- 認可が絡む場合のガード（ログイン前後の遷移）

## Phase 3: ドメイン基盤（API/DB/共通モジュール）
- DBスキーマ最小セット（主要テーブル）
- APIの型/DTO/バリデーション方針
- 認証（F-01）※依存が多いなら最優先
- 共通エラーハンドリング/ローディング/リトライ
...
P0	T-01	F-01	-	認証（ログイン/ログアウト/ガード）	なし		
P0	T-02	S-03	T-??	商品一覧画面（API接続 or モック）	API未完なら要		
P1	T-03	S-04	T-02	商品詳細画面（遷移/表示）	遷移先未完なら要		

※「モック要否」には「未実装の遷移先」「未完成API」どちらが理由かも書く

## Phase 5: 結合 & 仕様調整（バグ潰しが主）
- 主要導線の結合（ログイン→主要機能→完了）
- モック除去（S-MOCKの置換）
- 権限/境界値/エラー系の統合
- パフォーマンス/ページング/キャッシュ（必要なら）
- 仕様の微修正（変更履歴を残す）

## Phase 6: 品質担保（テスト/セキュリティ/運用）
- テスト観点作成（最低限のE2E/シナリオ）
- 重要機能の回帰テスト手順
- セキュリティ（脆弱性スキャン/権限確認/ログの秘匿）
- 運用手順（障害時、問い合わせ対応、ログの見方）
- リリースノート草案

## Phase 7: リリース準備 & リリース
- リリース判定（Go/No-Go条件）
- マイグレーション手順/リハーサル（stgで）
- 本番リリース手順（担当/時間/ロールバック）
- 監視強化（リリース直後のアラート）
- リリース実施
- 初動確認（主要導線/メトリクス）
